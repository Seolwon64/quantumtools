...
- Cityscape는 순수 상태의 밀도행렬 ρ = |ψ⟩⟨ψ|(`quantum.js`의 `densityMatrix()`)의 실수부/허수부를 각각 2ⁿ×2ⁿ 3D 막대그래프로 그린 별도의 Three.js 씬이다(OrbitControls로 회전 가능). 막대 높이는 그 시점 행렬의 최대 절댓값 기준으로 정규화하고, 양수는 토스 블루, 음수는 빨강으로 구분한다. Cityscape 씬은 처음 전환될 때 생성한다(숨겨진 상태의 컨테이너는 크기가 0이라 미리 만들면 카메라 비율이 깨짐).
...
- Q-sphere 배경은 Bloch 모드의 와이어프레임 대신, 옅은 반투명 구 채움 + 해밍 가중치별(0~n) 위도 링으로 그린다. X/Y 축과 라벨, Z축 실린더는 Q-sphere 모드에서 숨긴다(마커에서 뻗는 스템 라인이 그 역할을 대신함).
- 각 마커 옆에는 `|비트열⟩ 위상` 라벨을 표시한다(예: `|001⟩ π`). 위상은 π의 간단한 분수(예: π/2, 2π/3)로 근사 표기하고, 맞아떨어지지 않으면 `0.xxπ` 소수로 표기한다.
- 구 패널 오른쪽 위에는 위상→색상 범례(색상환 + 0/π/2/π/3π/2 눈금 + "Phase" 라벨)를 Q-sphere 모드에서만 표시한다.
- 구 패널 오른쪽 아래에는(경고 아이콘과 배타적으로 같은 자리) "State"/"Phase angle" 체크박스를 표시한다. State는 마커+라벨 표시 여부, Phase angle은 위상 색 스템 라인 표시 여부를 각각 독립적으로 토글한다.