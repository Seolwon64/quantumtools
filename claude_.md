...
- **관측 상태(observed)**: 향후 샘플링 결과에서 관측된 기저는 어떤 필터(영확률·top-N)로도 숨기지 않는다. 현재 샘플링 미구현이라 `main.js`는 빈 집합(`snapshot.observedStates ?? new Set()`)을 넘긴다 — 로직·테스트는 준비돼 있다.
- Density Matrix(cityscape) 뷰에서는 확률 필터가 무의미하므로 토글·푸터를 숨긴다. 토글/버튼 조작 시 `renderProbabilities(circuit.getSnapshot())`로 즉시 다시 그린다.
