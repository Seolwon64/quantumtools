...
- **이동 흔적(Trail):** 벡터가 애니메이션으로 이동할 때마다 지나간 경로를 빨간색(불투명도 90%, Tube 메쉬로 두껍게)으로 구 표면에 남긴다. WebGL은 `Line`의 두께를 대부분 무시하므로 얇은 선이 아닌 Tube 메쉬로 그린다. 재생(▶)을 다시 누르거나 회로 편집/큐비트 탭 전환/처음으로(⏮) 등 상태가 스냅되는 시점에는 흔적을 초기화한다.
