...
- **(b) 액센트 `--accent-3/6/9/10/11`** — 인터랙티브 전용 hue(파랑) **하나**. 재생 버튼·선택·활성 탭·포커스 링·진행 점선에만 쓰고 **게이트 카테고리에서는 완전히 제외**한다. `--accent-9`는 `--gray-1` 글자 대비 4.75로 AA를 넘긴다(순백이 아니라 `--gray-1`이 얹히므로 그 기준으로 맞췄다).
- **(c) 게이트 카테고리 — hue 3개 + 중립 3종.** Pauli=A(27°) · Phase=B(195°) · Rotations=C(310°) · Interaction=B계열(168°) · Multi/Control·Non-unitary·Advanced=중립.
  - **OKLCH에서 L을 고정하고 hue만 바꾼다** → 모든 유채색 카테고리의 지각 명도가 같아져 "Y가 RY보다 중요해 보이는" 착시가 사라진다. 중립 3종은 hue가 없어 이 착시와 무관하므로 **명도로 구분**한다(사용자 지시의 "Interaction은 B의 다른 명도"는 L 고정 원칙과 충돌해, 같은 계열 안에서 **hue 각도+채도**로 구분하는 쪽으로 조정했다).
  - **2단계 tier:** `-bg`/`-fg` = 팔레트(밝게) · `-c` = 회로(한 단계만 진하게). 회로 tier가 예전처럼 어둡지 않아 캔버스가 차분하다. **회로 블록 글자는 `--gray-12`** — tier가 밝아져 흰 글자는 대비 1.7로 AA에 한참 못 미친다. 배경 없는 ⊕·× 기호는 `--gate-fg`(진한 톤)를 쓴다.
- **[4] 데이터 영역:** 큐비트/고전 와이어·차트 축·격자는 `--gray-6~7`로 최대한 옅게, 채도는 **데이터(막대·노드·게이트 블록)만** 갖는다. 배경은 패널과 같은 `--gray-2`.
- **색은 JS에 하드코딩하지 않는다** — `js/tokens.js`의 `token()`/`tokenHex()`/`accentAlpha()`로 **CSS 변수에서 읽어** 정의를 style.css 한 곳에 둔다(3D 구·차트 색이 조용히 어긋나던 원인).
- **허용 예외 2가지:** ① `.phase-wheel`의 conic-gradient(위상 컬러맵은 별도 작업으로 미룸) ② `scene.js`의 `0xffffff` 4개 — **광원의 색**이라 팔레트 토큰으로 바꾸면 3D 장면 전체에 색조가 입혀진다.
