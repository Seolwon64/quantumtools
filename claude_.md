...
- **(a) 중립 램프 `--gray-1..12`** — 배경·테두리·글자. **(b) 액센트 하나(`--accent-*`, 파랑)** — 누를 수 있는 것 전용. **(c) 게이트 카테고리** — 아래 3.s.
...
### 3.s. 게이트 카테고리 색 — 카테고리당 **하나**
팔레트 칩과 회로 블록이 **같은 변수(`--cat-*`)** 를 참조한다. 예전에는 `-bg`(팔레트 파스텔)와 `-c`(회로 중간톤)로 나뉘어 있어 **놓기 전후로 색이 달라졌고**, "팔레트의 이 게이트 = 회로의 저 블록"이 성립하지 않았다.
- **규격: OKLCH `L = 0.62` 고정(7개 전부) · 유채색 `C = 0.135` · 중성 `C = 0.038`.**
- **L=0.62는 거의 유일한 답이다.** 두 제약이 서로 반대 방향으로 밀기 때문 — L=0.64면 패널 경계 대비가 2.90으로 3:1 미달, L=0.60이면 `--gray-12` 글자 대비가 4.27로 AA 미달.
- **판단 기준은 "예뻐 보이는가"가 아니라 두 수치다**: 경계 대비(블록 vs `--gray-2`) **≥ 3.0**(WCAG 1.4.11 비텍스트) · 글자 대비(`--gray-12` vs 블록) **≥ 4.5**(AA). 실측 3.24~3.67 / 4.64~5.26. `test/colors.test.mjs`가 고정한다.
- **청록 계열은 색역 밖이라 그냥 쓰면 L이 밀린다.** phase(195°)·interaction(168°)은 L=0.62에서 C=0.135가 sRGB를 벗어나 채널이 잘리고 L이 0.634로 올라간다 → **L·H는 고정하고 C만 줄여** 색역에 맞춘다(CSS Color 4의 gamut mapping). 그 결과 L 편차 0.0021.
- **흰 글자로 뒤집지 않는다.** 이 배경에서 흰 글자는 3.28~3.87로 AA 미달이다(테스트가 이 사실을 고정한다). `--gray-12`는 램프의 최대 농도라 더 어둡게 갈 곳도 없다.
- **중성 3계열(Multi·Non-unitary·Advanced)은 명도가 아니라 hue로 구분한다.** 명도로 나누면 L 균일이 깨져 특정 카테고리가 더 중요해 보인다. 채도를 0.038로 낮게 유지해 회색 성격은 지킨다.
- **배치 전후로 색을 바꾸는 필터를 두지 않는다.** 2타깃 게이트(RZZ·RXX)의 두 번째 블록만 `opacity: .75`로 흐리게 하던 규칙이 있었는데, **하나의 게이트가 두 색으로 보였다**. 삭제했다.
