...
- Q-sphere 배경은 Bloch 모드의 와이어프레임 대신, 조명을 받는 옅은 반투명 구 채움(MeshStandardMaterial + ambient/directional light로 은은한 입체 음영) + 해밍 가중치별(0~n) 위도 링 + 극-극 수직 중심축 + 항상 카메라를 향하는 외곽 실루엣 원(great-circle billboard)으로 그려 구 윤곽을 또렷하게 한다. X/Y 축과 라벨, Z축 실린더는 Q-sphere 모드에서 숨긴다(마커에서 뻗는 스템 라인이 그 역할을 대신함).
