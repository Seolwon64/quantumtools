...
- **게이트 정보 표시(`js/gatematrix.js` + `#gate-info`):** **컨텍스트 메뉴의 "Show info"로만** 연다. hover는 표준 이름 툴팁만(식별용 — 행렬 미리보기는 넣지 않는다), 좌클릭은 메뉴를 열 뿐 정보를 자동으로 띄우지 않는다.
  - **팔레트 게이트 우클릭**으로도 같은 패널을 연다(`infoTarget = { palette: gateName }`). 배치 전이라 제어·파라미터가 없으므로 기본 2×2를 보여주는데, **파라미터 게이트(RX/RY/RZ/P/U)는 대표값(π/2)으로 수치 행렬을 내면 거짓**이므로 `SYMBOLIC_MATRICES`(main.js의 표시용 문자열 표)로 **기호 행렬**을 그리고 "Place this gate to see its matrix with the chosen angle."를 덧붙인다. 좌클릭/드래그 경로는 그대로.
...
- **회로 블록 색(팔레트와 2단계 분리):** 회로 캔버스 칩은 `--cat-*-c`로 **불투명하게 채우고 흰 글자**를 쓴다. 설계 규칙 두 가지: ① **hue를 4계열로 축소**(빨강 Pauli / 파랑계열 Phase·Multi·Rotation을 hue 미세차로 / 청록 Interaction / 회색 Non-unitary·Advanced) — 한 회로가 알록달록해지지 않게. ② **흰 글자 대비비를 전부 4.80으로 통일** — 대비비는 배경 휘도의 함수라 이를 맞추면 **지각 밝기가 균일**해져 특정 게이트가 더 중요해 보이는 착시가 사라지고 AA(≥4.5)도 동시에 만족한다. `test/phase.test.mjs`가 두 규칙을 고정한다.
  - **2큐비트 게이트의 두 블록은 감광하지 않는다** — 예전엔 두 번째 타깃에 `.placed-partner{opacity:.75}`를 걸어 RZZ/RXX가 한쪽만 흐리게 나와 **서로 다른 게이트처럼 보였다**. 하나의 게이트는 같은 색이어야 한다.
  - `.placed-gate`가 `--gate-c`로 카테고리 색을 참조하고, 팔레트 버튼(`.gate-chip.cat-*`)은 파스텔 그대로다. 블록은 40px(팔레트 대비 +~18%), 셀 50px, 세로 피치 56 — 4~5큐비트가 세로 스크롤 없이, 7단계가 가로 스크롤 없이 들어간다.
...
### 4.7. 3D 시각 요소
- 축은 `Line` 대신 얇은 Cylinder 메쉬로 그려 두께/불투명도를 확실히 제어한다 (WebGL은 `Line`의 linewidth를 대부분 무시하기 때문).
- **Bloch 격자(`blochGridGroup`, Bloch 전용):** 예전엔 `wireframe:true` 구 하나였는데 경도선이 빽빽해 화살표를 가렸다. 지금은 **경도선을 없애고** ① 위도 링 여러 개 ② **적도만 더 진하게(+한 겹 덧그려 굵게)** ③ **x·y·z 축 기준 대원 2개**(xy·yz 평면) ④ 위도 사이 띠에 아주 옅은 음영(구면 밴드, opacity 0.045)으로 구성한다. **축 기준선은 반드시 남긴다** — 위상 φ는 수평 각도로 읽는 값이라 기준이 없으면 읽을 수 없다. 모드 전환은 `blochGridGroup.visible`. **Q-sphere의 위도 링은 해밍 무게 정보이므로 이 변경과 무관하게 그대로 둔다.**
...
- **상태 노드:** 각 계산기저를 광택 구슬(`MeshStandardMaterial` + emissive, 환경맵 반사)로 표면에 배치한다. 크기는 확률 비례, 색은 위상(아래 컬러맵), 중심에서 각 노드로 반투명 색상 스템 라인이 뻗는다.
- **노드 hover(`setNodeHoverHandler`):** `THREE.Raycaster`로 히트 테스트하되 **`mousemove`에선 좌표만 저장하고 `renderLoop`에서 프레임당 1회만** 검사한다(≤60fps 스로틀). 결과는 카메라 거리순이라 `[0]`만 보면 **겹친 노드 중 가장 앞의 것**만 잡힌다. 노드가 작아 겨냥이 어려우므로 마커마다 **반지름 ×1.9의 투명 히트 메쉬**(opacity 0)를 자식으로 붙인다. hover 시 마커를 확대(+emissive 강화)하고, 툴팁 내용(기저 켓·진폭·확률·위상 `π 분수 = 라디안`)은 main.js가 **가짜 anchor**(`getBoundingClientRect`만 구현)를 `showTooltip`에 넘겨 기존 툴팁 스타일을 재사용해 그린다.
- **위상 컬러맵(`js/phase.js`) — 색상환과 노드가 반드시 공유하는 단 하나의 정의.** 예전엔 노드는 JS의 HSL hue 보간, 색상환은 **CSS `conic-gradient` 하드코딩**이라 서로 달랐다(범례가 거짓말). 지금은 `phaseToRgb()` 하나에서 노드 색과 색상환 그라디언트가 모두 나온다.
  - **왜 Oklch인가:** 순수 HSL hue는 지각 밝기가 불균일해(노랑이 파랑보다 훨씬 밝다) 특정 위상이 강조된 것처럼 보이는 착시가 생기는데, 위상은 크기 개념이 없는 순환량이라 이건 **데이터 왜곡**이다. **L·C를 고정하고 hue만 0→360**으로 돌려 모든 위상의 지각 밝기를 동일하게 맞췄고(테스트가 휘도 비 <1.25로 고정), C를 낮춰 파스텔로 만들었다. 2π→0 순환성은 구조적으로 보장된다.
  - 색상환 UI는 **도넛(링)** 이고 가운데에 "Phase" 라벨이 들어간다. 마스크는 `.phase-wheel::before`에만 걸어야 한다(요소 자체에 걸면 라벨·눈금까지 잘린다).
