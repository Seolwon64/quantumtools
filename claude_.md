...
- 칼럼 비율은 내용에 맞게 비대칭이며, 위 행과 아래 행의 칼럼 비율은 서로 독립이다.
...
- 4개 패널의 크기는 드래그로 조정 가능하다. 스플리터는 총 3개: 위 행 칼럼 스플리터(팔레트↔회로), 아래 행 칼럼 스플리터(구↔확률), 행 스플리터(위↔아래). 행별 칼럼 스플리터가 독립적이라 Operations 패널만 따로 넓히는 것이 가능하다 (칼럼 22~60%, 로우 25~75% 범위 제한). 조정한 비율은 `localStorage`(`bloch-layout-v2`)에 저장해 새로고침 후에도 유지한다.
