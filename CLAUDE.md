...
| `grid.js` | 회로 그리드 DOM 생성 — 셀·연결선·스텝 인디케이터 배치 |
| `gatemenu.js` | 배치된 게이트의 컨텍스트 메뉴·선택 표시·게이트 정보 패널 |
...
- `js/*.js` 는 첫 줄에 그 모듈이 무엇을 담당하는지 한 줄 주석을 둔다. 새 모듈도 이
  관례를 따른다. 예외 없이 전 파일이 지키고 있다.
- 주석은 "무엇"보다 **"왜"** 를 쓴다. 과거에 실제로 틀렸던 것과 그때 내린 판단을
  남겨두는 식이다.
- 코드 위치는 줄 번호가 아니라 심볼로 가리킨다. **이는 오래 남는 것 — 문서, 주석,
  명세 — 에 적용된다.** 그 자리에서 소비되고 버려지는 보고서(리뷰·감사·디버그
  결과)는 줄 번호를 적어도 된다. 단 실측한 것만 적고 추측한 줄 번호는 쓰지 않는다.
...
무엇을 했는지보다 **그래서 무엇이 좋아지는지**를 담는 편이다. 커밋 하나가
...
- **Edit / Write 도구로 편집하면** PostToolUse 훅(`syntax-check.mjs`)이 돌아 문법 오류를
  차단한다. 훅이 거부하면 수정이 깨진 것이니 바로 고친다.
- **Bash 리다이렉션(`>>`, `tee`, `sed -i`, heredoc)은 그 훅을 타지 않는다.** matcher 가
  도구 이름(`Write|Edit`)에 걸리기 때문이다. `printf >> js/tokens.js` 로 파일을 파싱
  불가능하게 만들어도 훅이 뜨지 않았고 테스트 253개는 전부 통과했다 — 테스트가
  `tokens.js` 를 임포트하지 않기 때문이다.
- 그래서 턴이 끝날 때 Stop 훅(`run-tests.mjs`)이 **문법 전수 검사 후 테스트**를 돌린다.
  `js`·`test`·`.claude/hooks`·`.claude/skills`·`scripts` 아래 `.js`/`.mjs`/`.cjs` 전부에
  `node --check` 를 걸고, 하나라도 깨지면 턴을 되돌린다. 편집 경로와 무관하다.
- **논리를 바꾸는 Bash 편집은 여전히 아무 검사도 타지 않는다.** `sed -i` 로 상수 값을
  바꾸는 식은 문법이 멀쩡해 이 검사를 통과한다. 파일 편집은 Edit / Write 도구를 쓰고,
  Bash 로 고쳤다면 `git diff` 로 직접 확인한다.
