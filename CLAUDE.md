...
명세는 둘로 나뉘고, 분해 작업용 문서가 하나 더 있다. 필요한 것만 읽는다.
...
**`docs/main-js-decomposition.md`** — `js/main.js` 분해 작업(5·6단계와 그 뒤의 정리
커밋)을 할 때만. 단계 분할과 순서, 단계마다 돌릴 검증, 위험 지점, 발견해 두고 미뤄 둔
항목이 여기 있다. **그 외에는 읽지 않는다** — 제품 명세가 아니라 한 번의 리팩터링
기록이고, 분해가 끝나면 역할이 끝난다.
