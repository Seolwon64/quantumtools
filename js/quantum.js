...
export const GATE_INFO = {
  H: { label: "H", kind: "fixed", group: "hadamard", ready: true, desc: "Hadamard — 중첩 생성" },
  ...
  S: { label: "S", kind: "fixed", group: "phase", ready: true, desc: "S — π/2 위상" },
  Sdg: { label: "S†", kind: "fixed", group: "phase", ready: true, desc: "S† — −π/2 위상" },
  T: { label: "T", kind: "fixed", group: "phase", ready: true, desc: "T — π/4 위상" },
  Tdg: { label: "T†", kind: "fixed", group: "phase", ready: true, desc: "T† — −π/4 위상" },
  I: { label: "I", kind: "fixed", group: "pauli", ready: true, desc: "Identity — 항등" },
  SX: { label: "√X", kind: "fixed", group: "rotation", ready: true, desc: "√X — X의 제곱근" },
  SXdg: { label: "√X†", kind: "fixed", group: "rotation", ready: true, desc: "√X† — √X의 역연산" },
  RX: { label: "RX", kind: "param", group: "rotation", ready: true, defaultTheta: Math.PI / 2, desc: "RX — X축 회전" },
  RY: { label: "RY", kind: "param", group: "rotation", ready: true, defaultTheta: Math.PI / 2, desc: "RY — Y축 회전" },
  RZ: { label: "RZ", kind: "param", group: "phase", ready: true, defaultTheta: Math.PI / 2, desc: "RZ — Z축 회전" },
  P: { label: "P", kind: "param", group: "phase", ready: true, defaultTheta: Math.PI / 2, desc: "Phase — 위상 회전" },
  CNOT: { label: "CNOT", targetLabel: "⊕", kind: "controlled", base: "X", controls: 1, group: "pauli", ready: true, minQubits: 2, desc: "CNOT — 제어 NOT" },
  CZ: { label: "CZ", targetLabel: "Z", kind: "controlled", base: "Z", controls: 1, group: "control", ready: true, minQubits: 2, desc: "CZ — 제어 Z" },
  CCX: { label: "CCX", targetLabel: "⊕", kind: "controlled", base: "X", controls: 2, group: "pauli", ready: true, minQubits: 3, desc: "Toffoli (CCX) — 이중 제어 NOT" },
  SWAP: { label: "SWAP", targetLabel: "×", kind: "swap", group: "pauli", ready: true, minQubits: 2, desc: "SWAP — 두 큐비트 교환" },
  RXX: { label: "RXX", targetLabel: "RXX", kind: "pair-param", group: "rotation", ready: true, minQubits: 2, defaultTheta: Math.PI / 2, desc: "RXX — XX 상호작용 회전" },
  RZZ: { label: "RZZ", targetLabel: "RZZ", kind: "pair-param", group: "rotation", ready: true, minQubits: 2, defaultTheta: Math.PI / 2, desc: "RZZ — ZZ 상호작용 회전" },
  CTRL: { label: "•", kind: "dot", group: "structural", ready: true, desc: "Control — 같은 칼럼 게이트에 컨트롤 부여" },
  RCCX: { label: "RCCX", targetLabel: "⊕", kind: "controlled", base: "X", controls: 2, group: "rotation", ready: true, minQubits: 3, desc: "RCCX — 상대위상 Toffoli" },
  RC3X: { label: "RC3X", targetLabel: "⊕", kind: "controlled", base: "X", controls: 3, group: "rotation", ready: true, minQubits: 4, desc: "RC3X — 상대위상 3중 제어 NOT" },
  U: { label: "U", kind: "param3", group: "rotation", ready: true, defaultTheta: Math.PI / 2, desc: "U — 범용 단일 큐비트 회전 (θ, φ, λ)" },
  BARRIER: { label: "⋮", kind: "noop", group: "structural", ready: true, desc: "Barrier — 시각 구분선 (상태 불변)" },
  MEASURE: { label: "M", kind: "noop", group: "structural", ready: true, desc: "Measure — Z기저 측정 (확률 분포 확정)" },
  RESET: { label: "|0⟩", kind: "reset", group: "structural", ready: true, desc: "Reset — |0⟩로 초기화" },
  IF: { label: "if", kind: "noop", group: "structural", ready: false, desc: "If — 고전 조건부 실행 (측정 붕괴가 없어 미지원)" },
};
